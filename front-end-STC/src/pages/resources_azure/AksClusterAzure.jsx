...
import autoTable from 'jspdf-autotable'; // Este te deja usar autoTable(doc, ...)

export default function AksClustersAzure() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const result = await fetch(`http://localhost:8080/api/cloud/aksclustersazure?subscriptionId=${selectedSubscriptionId}`);
  ...
  const exportToCSV = () => {
    ...
    saveAs(blob, `AKS_Clusters_${subscriptionName}_${new Date().toISOString().split('T')[0]}.csv`);
  };
  ...
  const exportToPDF = () => {
    ...
    doc.text(`Azure AKS Clusters - ${subscriptionName}`, 14, 15);
    
    autoTable(doc, {
      head: [['ID', 'Name', 'Resource Group', 'Subscription ID', 'Location', 'Kubernetes Version', 'Status']],
    ...
    doc.save(`AKS_Clusters_${subscriptionName}_${new Date().toISOString().split('T')[0]}.pdf`);
  };
      ...
      <div className="flex flex-col h-screen">
        <div className="w-full flex flex-col flex-grow">
          <div className="flex justify-between mb-4 px-6">
            <h1 className="text-2xl text-[#0078D4] self-center">Azure AKS Clusters</h1>
            ...
            <table className="min-w-full bg-white rounded-lg">
              <thead>
                <tr className="bg-gray-200 text-[#004E8C] font-semibold">
                  ...
                  <td className="py-2 px-4 border-b border-gray-200 text-center">Kubernetes Version</td>
                ...
                {filteredData.length > 0 ? (
                  filteredData.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td className="text-sm text-center py-2 px-4 border-b border-gray-200">{item.kubernetesVersion}</td>
