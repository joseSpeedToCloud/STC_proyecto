  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const result = await fetch(`http://localhost:8080/api/cloud/snapshotsazure?subscriptionId=${selectedSubscriptionId}`);
      ...
      <div className="flex flex-col h-screen">
        <div className="w-full flex flex-col flex-grow">
          <div className="flex justify-between mb-4 px-6">
            <h1 className="text-2xl text-[#0078D4] self-center">Azure Snapshots</h1>
            ...
            <table className="min-w-full bg-white rounded-lg">
              <thead>
                <tr className="bg-gray-200 text-[#004E8C] font-semibold">
                  ...
                  <td className="py-2 px-4 border-b border-gray-200 text-center">Source Disk ID</td>
                  <td className="py-2 px-4 border-b border-gray-200 text-center">Size (GB)</td>
                  <td className="py-2 px-4 border-b border-gray-200 text-center">Storage Type</td>
                ...
                {filteredData.length > 0 ? (
                  filteredData.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td className="text-sm text-center py-2 px-4 border-b border-gray-200">{item.sourceDiskId}</td>
                      <td className="text-sm text-center py-2 px-4 border-b border-gray-200">{item.diskSizeGb}</td>
                      <td className="text-sm text-center py-2 px-4 border-b border-gray-200">{item.storageAccountType}</td>
