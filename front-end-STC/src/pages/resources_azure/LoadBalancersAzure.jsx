...
export default function LoadBalancersAzure() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const result = await fetch(`http://localhost:8080/api/cloud/loadbalancersazure?subscriptionId=${selectedSubscriptionId}`);
  ...
  const exportToCSV = () => {
    ...
    saveAs(blob, `Azure_LoadBalancers_${subscriptionName}_${new Date().toISOString().split('T')[0]}.csv`);
  };
  ...
  const exportToPDF = () => {
    ...
    doc.text(`Azure Load Balancers - ${subscriptionName}`, 14, 15);
    
    autoTable(doc, {
      head: [['ID', 'Name', 'Resource Group', 'Location', 'SKU', 'Frontend IP', 'Backend Pools', 'Status']],
    ...
    doc.save(`Azure_LoadBalancers_${subscriptionName}_${new Date().toISOString().split('T')[0]}.pdf`);
  };
      ...
      <div className="flex flex-col h-screen">
        <div className="w-full flex flex-col flex-grow">
          <div className="flex justify-between mb-4 px-6">
            <h1 className="text-2xl text-[#0078D4] self-center">Azure Load Balancers</h1>
            ...
            <table className="min-w-full bg-white rounded-lg">
              <thead>
                <tr className="bg-gray-200 text-[#004E8C] font-semibold">
                  ...
                  <td className="py-2 px-4 border-b border-gray-200 text-center">SKU</td>
                  <td className="py-2 px-4 border-b border-gray-200 text-center">Frontend IP</td>
                  <td className="py-2 px-4 border-b border-gray-200 text-center">Backend Pools</td>
                  <td className="py-2 px-4 border-b border-gray-200 text-center">Probe Protocol</td>
                  <td className="py-2 px-4 border-b border-gray-200 text-center">Probe Port</td>
                ...
                {filteredData.length > 0 ? (
                  filteredData.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td className="text-sm text-center py-2 px-4 border-b border-gray-200">{item.sku}</td>
                      <td className="text-sm text-center py-2 px-4 border-b border-gray-200">{item.frontendIpConfiguration}</td>
                      <td className="text-sm text-center py-2 px-4 border-b border-gray-200">{item.backendAddressPools}</td>
                      <td className="text-sm text-center py-2 px-4 border-b border-gray-200">{item.probeProtocol}</td>
                      <td className="text-sm text-center py-2 px-4 border-b border-gray-200">{item.probePort}</td>
