...
export default function DisksGoogle() {
  ...
  useEffect(() => {
    ...
    const exportToCSV = () => {
        const csvRows = [
          ['ID', 'Name', 'Project ID', 'Type', 'Zone', 'Size (GB)', 'Status'],
          ...filteredData.map(item => [
            item.disk_id, item.name, item.project_id, item.type, item.zone, item.size, item.status || ''
          ])
        ];
          const csvContent = csvRows.map(e => e.join(',')).join('\n');
          const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
          saveAs(blob, `Disks_${projectDisplayName}.csv`);
        };
      
        const exportToPDF = () => {
          const doc = new jsPDF();
          doc.text(`Disks del Proyecto: ${projectDisplayName}`, 10, 10);
          autoTable(doc, {
            head: [['ID', 'Name', 'Project ID', 'Type', 'Zone', 'Size (GB)', 'Status']],
            body: filteredData.map(item => [
              item.disk_id, item.name, item.project_id, item.type, item.zone, item.size, item.status || ''
            ]),
          });
          doc.save(`Disks_${projectDisplayName}.pdf`);
        };
    ...
    <Sidebar showBackButton={true}>
      ...
      <h1 className="text-3xl font-bold mb-6 text-[#3F9BB9]">
        ...
        <div className="w-full flex flex-col flex-grow">
          ...
          <h1 className="text-2xl mb-6 ml-6 text-[#3F9BB9]">Disks</h1>
