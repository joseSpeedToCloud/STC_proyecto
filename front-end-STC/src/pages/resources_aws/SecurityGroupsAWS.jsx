...
export default function SecurityGroupsAWS() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const result = await fetch(`http://localhost:8080/api/aws/securitygroupsAWS?accountId=${selectedAccountId}`);
      ...
      <div className="flex flex-col h-screen">
        <div className="w-full flex flex-col flex-grow">
          <div className="flex flex-col md:flex-row justify-between mb-4 px-6 gap-4">
            <h1 className="text-2xl text-[#FF9900] self-center">AWS Security Groups</h1>
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <ExportarDatosArchivos 
                data={filteredData}
                columns={columns}
                filename={`AWS_SecurityGroups_${accountName}`}
            ...
            <table className="min-w-full bg-white rounded-lg">
              <thead>
                <tr className="bg-gray-200 text-[#232F3E] font-semibold">
                  <td className="py-2 px-4 border-b border-gray-200 text-center">Group ID</td>
              ...
              <tbody>
                {filteredData.length > 0 ? (
                  filteredData.map((item, index) => (
                    <tr key={index}>
                      <td className="text-sm text-center py-2 px-4 border-b border-gray-200">{item.groupId}</td>
