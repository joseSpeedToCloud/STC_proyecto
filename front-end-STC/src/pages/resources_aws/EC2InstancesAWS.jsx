...
export default function EC2InstancesAWS() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const result = await fetch(`http://localhost:8080/api/aws/ec2instancesAWS?accountId=${selectedAccountId}`);
  ...
  const exportColumns = [
    ...
    { header: 'Public IP', accessor: item => item.publicIp || 'N/A' },
    { header: 'Private IP', accessor: item => item.privateIp || 'N/A' }
  ];
      ...
      <div className="flex flex-col h-screen">
        <div className="w-full flex flex-col flex-grow">
          <div className="flex justify-between mb-4 px-6">
            <h1 className="text-2xl text-[#FF9900] self-center">AWS EC2 Instances</h1>
            <div className="flex space-x-4">
              ...
              <ExportarDatosArchivos 
                ...
                filename={`AWS_EC2_Instances_${accountName}`}
                title={`AWS EC2 Instances - ${accountName}`}
            ...
            <table className="min-w-full bg-white rounded-lg">
              <thead>
                <tr className="bg-gray-200 text-[#232F3E] font-semibold">
                  <td className="py-2 px-4 border-b border-gray-200 text-center">Instance ID</td>
                  <td className="py-2 px-4 border-b border-gray-200 text-center">Name</td>
                  <td className="py-2 px-4 border-b border-gray-200 text-center">Instance Type</td>
                  ...
                  <td className="py-2 px-4 border-b border-gray-200 text-center">Public IP</td>
                  <td className="py-2 px-4 border-b border-gray-200 text-center">Private IP</td>
                </tr>
              </thead>
              <tbody>
                {filteredData.length > 0 ? (
                  filteredData.map((item, index) => (
                    <tr key={index}>
                      <td className="text-sm text-center py-2 px-4 border-b border-gray-200">{item.instanceId}</td>
                      <td className="text-sm text-center py-2 px-4 border-b border-gray-200">{item.name}</td>
                      <td className="text-sm text-center py-2 px-4 border-b border-gray-200">{item.instanceType}</td>
                      ...
                      <td className="text-sm text-center py-2 px-4 border-b border-gray-200">{item.publicIp || 'N/A'}</td>
                      <td className="text-sm text-center py-2 px-4 border-b border-gray-200">{item.privateIp || 'N/A'}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
