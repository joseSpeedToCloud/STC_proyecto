...
export default function EKSClustersAWS() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const result = await fetch(`http://localhost:8080/api/aws/eksclustersAWS?accountId=${selectedAccountId}`);
  ...
  const exportColumns = [
    ...
    { header: 'Role ARN', accessor: 'roleArn' }
  ];
      ...
      <div className="flex flex-col h-screen">
        <div className="w-full flex flex-col flex-grow">
          <div className="flex justify-between mb-4 px-6">
            <h1 className="text-2xl text-[#FF9900] self-center">AWS EKS Clusters</h1>
            <div className="flex space-x-4">
              ...
              <ExportarDatosArchivos 
                ...
                filename={`AWS_EKS_Clusters_${accountName}`}
                title={`AWS EKS Clusters - ${accountName}`}
                subtitle={`Total: ${filteredData.length} clusters`}
              />
            </div>
          </div>
            ...
            <table className="min-w-full bg-white rounded-lg">
              <thead>
                <tr className="bg-gray-200 text-[#232F3E] font-semibold">
                  ...
                  <td className="py-2 px-4 border-b border-gray-200 text-center">Version</td>
                  <td className="py-2 px-4 border-b border-gray-200 text-center">Status</td>
                  <td className="py-2 px-4 border-b border-gray-200 text-center">Role ARN</td>
                </tr>
              </thead>
              <tbody>
                {filteredData.length > 0 ? (
                  filteredData.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td className="text-sm text-center py-2 px-4 border-b border-gray-200">{item.version}</td>
                      <td className="text-sm text-center py-2 px-4 border-b border-gray-200">{item.status}</td>
                      <td className="text-sm text-center py-2 px-4 border-b border-gray-200">{item.roleArn}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
